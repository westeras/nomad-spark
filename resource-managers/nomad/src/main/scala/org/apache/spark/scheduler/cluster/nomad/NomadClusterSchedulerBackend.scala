...
import org.apache.spark.scheduler.cluster.CoarseGrainedClusterMessages.{RegisterExecutor, RequestExecutors, StopDriver}
...
private[spark] class NomadClusterSchedulerBackend(
  ...
  protected class NomadDriverEndpoint(rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = ({
       ...
       */
      case RequestExecutors(total, _, _) =>
  ...
  override protected def doRequestTotalExecutors(requestedTotal: Int): Future[Boolean] =
    driverEndpoint.ask[Boolean](RequestExecutors(requestedTotal, 0, Map.empty))
